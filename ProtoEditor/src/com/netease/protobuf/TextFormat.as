...
package com.netease.protobuf {
	...
	public final class TextFormat {
		...
		private static const SINGLELINE_MODE:PrintSetting = new PrintSetting(' '.charCodeAt(), "", ":");
		private static const MULTILINE_MODE:PrintSetting = new PrintSetting('\n'.charCodeAt(),"  ", ": ");
		private static function printEnum(output:IDataOutput, value:int, enumType:Class):void
		{
			Platform.printEnum(output, value, enumType, allEnumValues);
		}
		...
		public static function printUnknownField(output:IDataOutput, tag:uint,
				value:Object, printSetting:PrintSetting, currentIndent:String):void {
			const unknownArray:Array = typeAs(value , Array);
			if (unknownArray) {
		...
		{
			Platform.printMessageFields(output, message, printSetting, currentIndent, allMessageFields);
		}
		
		public static function printValue(output:IDataOutput,
				fieldDescriptor:BaseFieldDescriptor,
				value:Object,
				printSetting:PrintSetting,
				currentIndent:String = ""):void {
			const message:Message = typeAs(value , Message)
			if (message) {
				...
				output.writeUTFBytes(printSetting.simpleFieldSeperator)
				const stringValue:String = typeAs(value , String);
				if (stringValue !== null) {
					printString(output, stringValue)
				} else {
					const enumFieldDescriptor:FieldDescriptor$TYPE_ENUM =
						typeAs(fieldDescriptor , FieldDescriptor$TYPE_ENUM)
		...
		private static function appendUnknown(message:Message, tag:uint,
			...
			if (oldValue === undefined) {
				...
				const oldArray:Array = typeAs(oldValue , Array)
				if (oldArray) {
					oldArray.push(value)
				} else {
		...
		private static function consumeField(source:ISource,
			...
			const repeatedFieldDescriptor:RepeatedFieldDescriptor =
				typeAs(fieldDescriptor , RepeatedFieldDescriptor)
